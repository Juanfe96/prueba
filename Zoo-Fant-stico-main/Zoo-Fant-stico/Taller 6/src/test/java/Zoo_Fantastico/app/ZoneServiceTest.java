...
public class ZoneServiceTest {
    ...
    void testCreateZone_ShouldReturnSavedZone() {
        ...
        zone.setDescription("A pelear en el Valhalla para revivir y comer un bufete!!!");
    ...
    @Test
    void testGetZoneById_ShouldReturnZone_WhenZoneExists() {
        // Arrange
        Long id = 1L;
        Zone zone = new Zone();
        zone.setId(id);
        ...
        // Act
        Zone retrievedZone = zoneService.getZoneById(id);

        // Assert
        assertNotNull(retrievedZone);
        assertEquals(id, retrievedZone.getId());
        verify(zoneRepository, times(1)).findById(id);
    }

    @Test
    void testGetZoneById_ShouldThrowException_WhenZoneDoesNotExist() {
        // Arrange
        Long id = 1L;
    ...
    void testUpdateZone_ShouldUpdateZone_WhenZoneExists() {
        ...
        // Act
        Zone resultZone = zoneService.updateZone(id, updatedZone);

        // Assert
        assertNotNull(resultZone);
        assertEquals("Paraiso Pastafarista", resultZone.getName());
        verify(zoneRepository, times(1)).findById(id);
        verify(zoneRepository, times(1)).save(any(Zone.class));
    }

    @Test
    void testUpdateZone_ShouldThrowException_WhenZoneDoesNotExist() {
        // Arrange
        Long id = 1L;
        Zone updatedZone = new Zone();
    ...
    @Test
    void testDeleteZone_ShouldDelete_WhenPoblationIsZero() {
        // Arrange
        Long id = 1L;
        Zone zone = new Zone();
        zone.setId(id);
        zone.setCapacity(6);
        zone.setPoblation(0); // Solo elimina si la población es 0
    ...
    @Test
    void testDeleteZone_ShouldThrowException_WhenPoblationIsMoreThanZero() {
        // Arrange
        Long id = 1L;
        Zone zone = new Zone();
        zone.setId(id);
